...
=======
  ...
  word_document:
    toc: true
    toc_depth: '3'
>>>>>>> 33317d74bb75aadd2c2ad9dbef03030be3734c3c
---
...
<<<<<<< HEAD
  summarise(kilos_totales = sum(kilos_vendidos, na.rm = TRUE),
    veces_comprado = n()) 
=======
>>>>>>> d47bda05c5aeda6f3818c0935aa3b3fc17d01279
...
=======
Tras analizar los datos obtenidos y resolver las preguntas anteriores nos hemos planteado las siguientes preguntas: 
>>>>>>> d47bda05c5aeda6f3818c0935aa3b3fc17d01279
...
mediana. Observamos los resultados obtenidos en una tabla.
...
¿Cuál es el producto más caro comprado?
...
Mostramos en un gráfico los productos más caros. 
...
Vemos que el producto más caro es el ALISTADO MEDIANO.
...
¿Cómo ha evolucionado el gasto a lo largo del tiempo?

```{r}

# Creamos un data.frame con la fecha y el total gastado en cada ticket
evolucion_gasto <- data.frame(
...
for (ticket in todos_los_tickets) {
  if (!is.null(ticket$productos) && !is.null(ticket$fecha)) {
    
    # Adaptamos el formato según el tipo de fecha
    fecha_ticket <- as.Date(ticket$fecha, format = "%d/%m/%Y")
    
    total_ticket <- sum(ticket$productos$importe, na.rm = TRUE)
    
    evolucion_gasto <- rbind(
      evolucion_gasto,
      data.frame(fecha = fecha_ticket, total = total_ticket, stringsAsFactors = FALSE)
    )
  }
}
```
...
Mostramos en un gráfico la evolución del gasto.
```{r}

# Ordenamos por fecha
evolucion_gasto <- evolucion_gasto[order(evolucion_gasto$fecha), ]

# Dibujamos la evolución del gasto
ggplot(evolucion_gasto, aes(x = fecha, y = total)) +
  geom_line(color = "steelblue", size = 1) +
  geom_point(color = "darkblue") +
  labs(title = "Evolución del gasto a lo largo del tiempo",
       x = "Fecha del ticket",
       y = "Total gastado (€)") +
  theme_minimal()
...
Este análisis nos permitiría identificar patrones en el consumo y detectar posibles fechas 
clave en las que el gasto aumenta. Pero, dado que el gráfico que obtenemos no sigue una 
tendencia definida, sino que muestra altibajos frecuentes podemos asumir que esto puede 
estar relacionado con compras puntuales de productos caros o con una planificación 
irregular de las compras.



### Pregunta 12

¿Cuál es el método de pago más frecuente en los tickets?
```{r}

# Extraemos todos los métodos de pago de los tickets
metodos_pago <- sapply(todos_los_tickets, function(ticket) ticket$metodo_pago)

# Creamos la tabla de frecuencia de métodos de pago
tabla_metodos <- table(metodos_pago)
```

Mostramos en un gráfico el método de pago más frecuente.
```{r}

# Convertimos la tabla a un data.frame 
df_metodos <- as.data.frame(tabla_metodos)

# Renombramos columnas para mayor claridad
colnames(df_metodos) <- c("Metodo_Pago", "Frecuencia")

# Creamos el gráfico
ggplot(df_metodos, aes(x = Metodo_Pago, y = Frecuencia, fill = Metodo_Pago)) +
  geom_bar(stat = "identity") +
  labs(title = "Método de Pago Más Utilizado en Tickets",
       x = "Método de Pago",
       y = "Frecuencia") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
```
Al analizar los métodos de pago en los tickets procesados, observamos que todos los 
tickets presentan "Tarjeta" como forma de pago. Esto sugiere que nuestro conjunto 
de tickets proviene de compras exclusivamente con tarjeta. Este resultado puede 
indicar un patrón de comportamiento del consumidor que muestra una preferencia clara 
por el uso de tarjeta frente al efectivo.
