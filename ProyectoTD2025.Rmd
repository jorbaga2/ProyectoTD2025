...
#Creamos dentro de la lista para almacenar los datos, un dataframe que va a contener los productos del ticket.
  ...
  datos$factura_simplifcada <- trimws(gsub("FACTURA SIMPLIFICADA:", "", linea_factura_simplificada))  
  ...
  # Función para extraer productos a peso (por ejemplo, peso y precio por kilo)
  extraer_producto_peso <- function(linea_actual, linea_siguiente) {
    ...
    if (!is.na(match_peso[1,1])) {
     cantidad <- 1
      descripcion <- paste(trimws(linea_actual), trimws(linea_siguiente))
      importe <- as.numeric(gsub(",", ".", match_peso[1,4]))
      return(data.frame(cantidad = cantidad, descripcion = descripcion, importe = importe, stringsAsFactors = FALSE))
   }
    return(NULL)
  }
...
# Ver el primer ticket
print(todos_los_tickets[[1]])

str(todos_los_tickets[[1]], max.level = 2)
